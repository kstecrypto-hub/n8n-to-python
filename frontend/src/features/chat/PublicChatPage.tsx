...
import { loadHiveWorkspaceDataset } from "@/lib/api/hiveWorkspace";
...
import {
  ...
  emptyApiary,
  emptyHive,
  reportScopedQuestion,
  seedMessages,
  type Apiary,
  type Hive,
  type HiveReport,
  type HiveStatus,
} from "@/features/chat/hiveWorkspaceDomain";
...
export function PublicChatPage() {
  ...
  async function handleSend() {
    ...
    setBusy(true);
    try {
        const response = await sendPublicChat({
          question: reportScopedQuestion(PUBLIC_TENANT_ID, activeReport, selectedApiary, selectedHive, question),
          query_mode: effectiveQueryMode,
          workspace_kind: "hive",
        });
      const answer = response.answer?.trim() || "The model returned without an answer body.";
      setMessages((current) => [...current, { id: createId("msg"), role: "assistant", content: answer, citations: response.citations, queryRunId: response.query_run_id ?? null }]);
