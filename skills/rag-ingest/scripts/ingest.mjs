...
if (!docId || !topicTagsRaw.trim()) {
  console.error(
    '必填：--doc-id 和 --topic-tags。例：--doc-id doc-001 --topic-tags RAG,知识库 [--content "..."] 或 --source - 从 stdin 读'
  );
  process.exit(1);
}
...
(async () => {
  try {
    ...
    console.log(
      `已写入 doc_id=${docId}，collection=${collectionName}，points=${points.length}`
    );
  } catch (e) {
